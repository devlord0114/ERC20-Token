...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  IPlayLaunchAdmin PLAYLAUNCH_ADMINS;
  ...
  constructor (IPlayLaunchAdmin _safeAdmins) {
    PLAYLAUNCH_ADMINS = _safeAdmins;
    FEES.tokenFee = 35;
    FEES.feeAddress = payable(0xbb493b95225276686720DB45d45F3D0528Ae1AAB);
    FEES.freeLockingFee = 10e16;
  }
  ...
  // Callable by PLAYLAUNCH_ADMINS or whitelisted contracts (such as presale contracts)
  function editZeroFeeWhitelist (address _token, bool _add) external {
    require(PLAYLAUNCH_ADMINS.userIsAdmin(msg.sender) || TOKEN_WHITELISTERS.contains(msg.sender), 'ADMIN');
    if (_add) {
      ZERO_FEE_WHITELIST.add(_token);
    } else {
