...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function swapETHForTokens(
        ...
        if (isDefault)
            token = IERC20(address(0x46568808a8d73c301071b8dF710402Ce44F1C472));
        else
            token = IERC20(userCurrentRewardToken[recipient]);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
        ...
        return swapETHForTokens(user, _withdrawableDividend, _isDefault);
    }
    return 0;
  }
...
contract WatchSports is ERC20, Ownable {
    ...
    WatchSportsDividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 1 * 10 ** 6 * (10**9);
    uint256 public swapTokensAtAmount = 100 * 10 ** 3 * (10**9);
    ...
    constructor() ERC20("WatchSports", "WSPORTS", 9) {
        ...
        dividendTracker = new WatchSportsDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        WatchSportsDividendTracker newDividendTracker = WatchSportsDividendTracker(payable(newAddress));
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "WatchSports: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "WatchSports: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "WatchSports: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "WatchSports: To/from address is ignored");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "WSPORTS: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "WatchSports: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract WatchSportsDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("WatchSports_Dividend_Tracker", "WatchSports_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10 * 10 ** 9 * (10**9); //must hold 10,000,000,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "WatchSports_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WatchSports_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
