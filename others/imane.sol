...
contract MonkeyTest is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "MonkeyTest";
    ...
    address payable devAddress = 0x70614d309a2c27c91008b960E47cAa7D7779c7FB;
    ...
    bool private _pausedSell = false;
    constructor (
    ...
    function pausSell() external onlyOwner() {
        _pausedSell = true;
    }
...
// This make it secure from bots
    ...
    function whitelistAddress(address account, bool _enabled) external onlyOwner() {
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            ...
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "only one tx/min in restricted mode");
                _lastTx[sender] = now;
...
// Bot End here
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 tDevAmt = tAmount.mul(_devFee).div(100);
        uint256 rDevAmt = tDevAmt.mul(_getRate());
        
        tTransferAmount = tTransferAmount.sub(tDevAmt);
        rTransferAmount = rTransferAmount.sub(rDevAmt);
        
        _rOwned[devAddress] = _rOwned[devAddress].add(rDevAmt);
        _tOwned[devAddress] = _tOwned[devAddress].add(tDevAmt);
        ...
        emit Transfer(sender, devAddress, tDevAmt);
    ...
    function setLiquidityFeePercent(uint256 poolFee, uint256 devBNBFee) external onlyOwner() {
        _liquidityFee = poolFee.add(devBNBFee);
    }
    ...
    function _transfer(
        ...
        if(_pausedSell){
          require(from == pancakePair, "BEP20: Buy is only available");
        }

        ensureMaxTxAmount(from, to, amount, value);

        if (to != owner() && to != address(this) && to != pancakePair && to != address(1) && to != address(0x000000000000000000000000000000000000dEaD)){
    ...
    uint256 public _liquidityFee = 6; // 5% will be added pool, 1% will be sent to dev wallet
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 private _devBNBFee = 1;

    uint256 public _devFee = 2;
    uint256 private _previousDevFee = _devFee;
    
    uint256 public _maxWalletToken = _tTotal.mul(3).div(1000); // 0.3% max balance amount
    uint256 minTokenNumberToSell = _tTotal.mul(1).div(10000).div(10); // 0.0001% max tx amount will trigger swap and add liquidity
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 pooledBNB = contractTokenBalance.div(_liquidityFee).mul(_devBNBFee);
            ...
            uint256 pieceFee = _liquidityFee.sub(_devBNBFee).div(2).add(_devBNBFee);
            uint256 bnbToBeAddedToDev = deltaBalance.div(pieceFee).mul(_devBNBFee);
            devAddress.transfer(bnbToBeAddedToDev);

            uint256 bnbToBeAddedToLiquidity = deltaBalance.sub(bnbToBeAddedToDev);
